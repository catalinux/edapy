...
def get_lin_model(name, XX, yy, estimators=1):
    ...
    m = lin_reg = LinearRegression()
    m.fit(X_train_rf, y_train)
    y_pred_test = m.predict(X_test_rf)
    return get_model_stats(name, y_test, y_pred_test)


modelStats = get_lin_model('lin_simple', X, y, 100)
x.append(modelStats)

modelStats = get_lin_model('lin_log10', X, np.log10(y), 100)
x.append(modelStats)
modelStats = get_lin_model('lin_sqrt', X, np.sqrt(y), 400)
x.append(modelStats)
modelStats = get_lin_model('xg_400_log', X, np.log(y), 400)
x.append(modelStats)
...
for i in range(len(comb)):
    select = comb[i]
    modelStats = get_lin_model('lin_comb' + str(i), X[select], y, 400)
    x.append(modelStats)
