...
X_rf = X_rf_pipeline.fit_transform(X)  # Whole set ran through pipeline for cross-val
X_train_rf = X_rf_pipeline.transform(X_train)
X_test_rf = X_rf_pipeline.transform(X_test)

m = RandomForestRegressor(n_estimators=1, bootstrap=False, n_jobs=3)
m.fit(X_train_rf, y_train)
y_pred_train = m.predict(X_train_rf)
y_pred_test = m.predict(X_test_rf)
x.append(get_model_stats('rf', y_test, y_pred_test))

m = RandomForestRegressor(n_estimators=400, bootstrap=False, n_jobs=3)
m.fit(X_train_rf, y_train)
y_pred_test = m.predict(X_test_rf)
x.append(get_model_stats('rf400', y_test, y_pred_test))

pd.DataFrame.from_dict(x).to_clipboard()
...
m = RandomForestRegressor(n_estimators=400, min_samples_split=0.1, max_features='sqrt', max_depth=18, bootstrap=False,
                          n_jobs=3)
m.fit(X_train_rf, y_train)
y_pred_test = m.predict(X_test_rf)
x.append(get_model_stats('rftune', y_test, y_pred_test))

pd.DataFrame.from_dict(x).round(2).to_clipboard()


def get_rf_model(name, XX, yy, estimators=1):
    ...
    m = RandomForestRegressor(n_estimators=estimators, min_samples_split=0.1, max_features='sqrt', max_depth=18,
...
modelStats = get_rf_model('rf_100_log10', X, np.log10(y), 100)
x.append(modelStats)
modelStats = get_rf_model('rf_400_log10', X, np.log10(y), 400)
x.append(modelStats)
modelStats = get_rf_model('rf_400_log', X, np.log(y), 400)
x.append(modelStats)
...
for i in range(len(comb)):
    select = comb[i]
    modelStats = get_rf_model('rf_comb' + str(i), X[select], y, 400)
    x.append(modelStats)
