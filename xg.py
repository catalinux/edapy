...
def get_xg_model(name, XX, yy, estimators=1):
    ...
    m = xgb.XGBRegressor(objective='reg:linear', colsample_bytree=0.3, learning_rate=0.45,
                         max_depth=7, alpha=10, n_estimators=estimators)
...
modelStats = get_xg_model('xg_100_log10', X, np.log10(y), 100)
x.append(modelStats)
modelStats = get_xg_model('xg_400_log10', X, np.log10(y), 400)
x.append(modelStats)
modelStats = get_xg_model('xg_400_log', X, np.log(y), 400)
x.append(modelStats)
...
for i in range(len(comb)):
    select = comb[i]
    modelStats = get_xg_model('xg_comb' + str(i), X[select], y, 400)
    x.append(modelStats)
